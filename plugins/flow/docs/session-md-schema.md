...
continuity** artifact used by `flow` skills to resume interrupted work and
...
> [Workflow State Contract](workflow-state-contract.md), which defines `.flow/state.json`
...
`.flow/state.json`, documented independently in
...
`.flow/state.json`, while SESSION.md and HANDOFF.json remain
advisory only.
...
(`.flow/state.json`) defined in the
[Workflow State Contract](workflow-state-contract.md).

| Concern | Session continuity | Workflow state |
|---------|-------------------|----------------|
| **Artifacts** | `.agent/SESSION.md`, `.agent/HANDOFF.json` | `.flow/state.json` |
...
3. A session that updates workflow progress (e.g., completing a phase) should update
   `.flow/state.json` through the owning workflow skill, not by modifying session
   continuity fields.
4. The two artifact families have independent discovery rules. Session continuity uses
   ...
   `.flow/state.json` with no fallback chain.
