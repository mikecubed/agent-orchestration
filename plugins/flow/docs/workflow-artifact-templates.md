...
- store them under `.flow/artifacts/`;
...
- `.flow/artifacts/direct-execution-<topic>.md`
- `.flow/artifacts/track-report-<topic>.md`
- `.flow/artifacts/review-resolution-<topic>.md`
- `.flow/artifacts/readiness-report-<topic>.md`
- `.flow/artifacts/publish-summary-<topic>.md`
- `.flow/artifacts/conductor-summary-<topic>.md`
- `.flow/artifacts/completion-summary-<topic>.md`
- `.flow/artifacts/refresh-summary-<topic>.md`
- `.flow/artifacts/batch-summary-<topic>.md`
...
should inspect `.flow/artifacts/` directly or follow an
explicit reference from `.flow/state.json` rather than
...
Use for `deliver` when the selected route is **direct**.
...
Artifact sink: <.flow/artifacts/direct-execution-<topic>.md or another durable sink>
Review handoff:
...
Readiness source: <pr-ready verdict for the exact tree being published>
...
Artifact sink: <.flow/artifacts/publish-summary-<topic>.md or another durable sink>
Next action: <what should happen next>
```

## Conductor lifecycle summary

Use for `idea-to-done`.

```text
Request: <clarified work request or accepted task bundle>
Progression mode: manual | guided | auto
Lifecycle owner: idea-to-done
State file: .flow/state.json
Entry path: <brainstorm | plan | deliver>
...
Artifact sink: <.flow/artifacts/conductor-summary-<topic>.md or another durable sink>
Next action: <what should happen next>
...
Use for final post-merge closeout in `idea-to-done`.
...
Artifact sink: <.flow/artifacts/completion-summary-<topic>.md or another durable sink>
Next action: <what should happen next>
...
**Prior-learnings lookup**: When the workflow supports prior-learning reuse (e.g., `plan`, `diff-review`), record the lookup result in the `Prior-learnings consulted` field. When matches are found, list each artifact as a one-line bullet with its path or reference and a short summary. Use `none-found` when the lookup ran but returned no matches, and `skipped` when the lookup was not performed (e.g., no knowledge sink is configured).
...
Use for `knowledge-save` and any workflow that captures a reusable lesson from a completed task.
...
Routed to knowledge-save:
- <gap description> — <evidence or context provided>, or none
Routed to arch-review:
- <candidate description> — <reason>, or none
Unresolved questions:
- <question or ambiguity>, or none
Artifact sink: <.flow/artifacts/refresh-summary-<topic>.md or another durable sink>
Next action: <what should happen next>
