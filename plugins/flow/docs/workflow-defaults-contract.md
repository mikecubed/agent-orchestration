...
`flow`. It gives repositories one narrow place to declare
...
```json
{
  ...
  "artifact-sinks": {
    "planning": "docs/",
    "track-reports": ".flow/artifacts/",
    "readiness-reports": ".flow/artifacts/",
    "publish-summaries": ".flow/artifacts/",
...
sink behavior. For example, `knowledge-save` may still require an explicit
...
1. a directory path such as `.flow/artifacts/` means "write the workflow's normal artifact
