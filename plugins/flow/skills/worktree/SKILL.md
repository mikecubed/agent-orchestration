---
name: worktree
description: Manage git worktree lifecycle for parallel track execution, reducing friction for flow:parallel-impl.
---

## Purpose

Use this skill to create, validate, and clean up git worktrees on behalf of `/flow:parallel-impl` or any other workflow that needs isolated working directories for parallel tracks.
...
- `/flow:parallel-impl` needs to provision work surfaces before launching tracks;
...
Skill invocation: /flow:worktree
```
...
The caller (`/flow:parallel-impl`) now dispatches implementers to each worktree path.
